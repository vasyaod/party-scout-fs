...
1. **Verify every event.** Each event must tie to a concrete source (ticket /
   ...
   (daytime)".)
1a. **`links` is an ordered list; index 0 is the highest priority.** Every event's
   info links live in a `links` **array** (replaces the old single `link`;
   see MODEL.md) — the **Open** button uses `links[0]`. The list is
...
8. **No raw URLs as text.** `links`, `tickets`, `maps` live in the JSON and are
...
10. **Open → the event website** (`links[0]`): event info/lineup page.
11. **No circular / source links.** Never store a scrape source (e.g. `19hz.info`) in
    an event's `links` list — it just points back at our own aggregator. Resolve
    the event's real page (RA / Tixr / Eventbrite / venue / official site) or leave
    `links` empty (no Open button). Same for any other listing index we pull from.
...
19. **Read the event page → write `summary` + `why`.** Visit every event's own page
    (`links[0]`/`tickets`) via an **ephemeral remote-browser session** (real Chrome;
