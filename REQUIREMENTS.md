...
1. **Verify every event.** Each event must tie to a concrete source (ticket /
   ...
   (daytime)".)
1a. **`source` is an ordered list; index 0 is the highest priority.** Every event's
   source/info links live in a `source` **array** (replaces the old single `link`;
   see MODEL.md) — the **Open** button uses `source[0]`. The list is
   **priority-ordered: the FIRST source is the most authoritative** (the anchor we
   trust most; e.g. the RA/venue listing). Put the primary source first. Scrape
   sources (19hz) are never stored. On merge the order is preserved (first wins) and
   new sources are appended; `[]` when none is known.
...
8. **No raw URLs as text.** `source`, `tickets`, `maps` live in the JSON and are
...
10. **Open → the event website** (`source[0]`): event info/lineup page.
11. **No circular / source links.** Never store a scrape source (e.g. `19hz.info`) in
    an event's `source` list — it just points back at our own aggregator. Resolve
    the event's real page (RA / Tixr / Eventbrite / venue / official site) or leave
    `source` empty (no Open button). Same for any other listing index we pull from.
...
19. **Read the event page → write `summary` + `why`.** Visit every event's own page
    (`source[0]`/`tickets`) via an **ephemeral remote-browser session** (real Chrome;
